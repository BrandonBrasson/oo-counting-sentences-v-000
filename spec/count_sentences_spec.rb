 describe String do
  ...
  puts  it "returns false if the string that you are calling this method on does NOT end in a exclamation mark." do
      expect("Hi, my name is Sophie".exclamation?).to eq(false)
    end

  end

  puts describe "#count_sentences" do

    puts it  "returns the number of sentences in a string" do
      expect("one. two. three?".count_sentences).to eq(3)
    end

    puts it "returns zero if there are no sentences in a string" do
      expect("".count_sentences).to eq(0)
    end

    puts it "returns the number of sentences in a complex string" do
